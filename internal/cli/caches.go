...
func newCachesCmd(rf *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(
		newCachesLsCmd(rf),
		newCachesRmCmd(rf),
		newCachesPruneCmd(rf),
	)
	return cmd
}

func newCachesLsCmd(rf *rootFlags) *cobra.Command {
	var quiet bool
	cmd := &cobra.Command{
		Use:     "ls [hash]",
...
func newCachesPruneCmd(rf *rootFlags) *cobra.Command {
	var force bool
	cmd := &cobra.Command{
		Use:   "prune",
		Short: "Remove all sandbox cache volumes",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cli, err := docker.NewClient("")
			if err != nil {
			...
			if len(targets) == 0 {
				fmt.Fprintln(out, "No sandbox cache volumes to prune.")
				return nil
			}
